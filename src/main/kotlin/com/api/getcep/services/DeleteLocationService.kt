...
@Service
class DeleteLocationService(private val locationRepository: LocationRepository, private val getLocationByIdService : GetLocationByIdService, private val producer: LocationProducer) {
    fun deleteLocation(idLocation: Long) {
        val location = getLocationByIdService.getLocationById(idLocation).toLocationEntity()
        locationRepository.delete(location)
        producer.send("Location deletada com ID: ${location.idLocation}", LocationProducer.Operation.DELETE)
    }
}